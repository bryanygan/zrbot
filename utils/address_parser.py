...
def parse_address(text: str) -> dict:
    ...
    for raw in text.splitlines():
        ...
        # Strip trailing phone numbers (e.g. ", 843-441-8687")
        cleaned = re.sub(r',?\s*\d{3}[-.]?\d{3}[-.]?\d{4}\s*$', '', cleaned).strip()
        ...
        if cleaned.lower() in COUNTRY_LINES:
            continue
        # Strip trailing country text (e.g. "... 91702 United States" → "... 91702")
        for country in sorted(COUNTRY_LINES, key=len, reverse=True):
            pattern = r'\b' + re.escape(country) + r'\s*$'
            m = re.search(pattern, cleaned, re.IGNORECASE)
            if m:
                stripped = cleaned[:m.start()].rstrip(' ,')
                if stripped:
                    cleaned = stripped
                    break
...
async def _get_usps_token(consumer_key: str, consumer_secret: str) -> str:
    """Obtain (or reuse) an OAuth2 bearer token from USPS."""
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, data=data) as resp:
            if resp.status != 200:
                ...
                raise RuntimeError(f"USPS OAuth failed ({resp.status}): {text}")
...
async def validate_address_usps(
    parsed: dict,
    consumer_key: str,
    consumer_secret: str,
) -> dict:
    """Validate and correct a parsed address via the USPS Addresses v3 API.

    Takes a parsed address dict (from ``parse_address``) and returns a new dict
    with USPS-corrected values.  The ``name`` field is preserved from the
    original since USPS does not handle recipient names.

    On API errors the original ``parsed`` dict is returned unchanged so the
    caller always has a usable result.
    """
    if "error" in parsed:
        return parsed
    ...
    try:
        ...
        logger.warning("USPS token error, returning local parse: %s", exc)
        return parsed

    # USPS API only accepts 5-digit ZIP codes
    zip5 = parsed["zip"][:5] if parsed.get("zip") else ""
    params = {
    ...
    if parsed.get("street2"):
        params["secondaryAddress"] = parsed["street2"]
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if resp.status != 200 or "error" in body:
                    logger.warning(
                        "USPS validation failed, returning local parse: %s",
                        body.get("error", {}).get("message", resp.status),
                    )
                    return parsed
    except Exception as exc:
        logger.warning("USPS request error, returning local parse: %s", exc)
        return parsed

    addr = body.get("address", {})
    return {
        "name": parsed.get("name", ""),
        "street": _smart_title(addr.get("streetAddress", parsed["street"])),
        "street2": _smart_title(addr.get("secondaryAddress", "") or parsed.get("street2", "")),
        "city": _smart_title(addr.get("city", parsed["city"])),
        "state": addr.get("state", parsed["state"]),
        "zip": addr.get("ZIPCode", parsed["zip"]),
